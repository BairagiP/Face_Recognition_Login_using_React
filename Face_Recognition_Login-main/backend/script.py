...
def get_data():
    global db
    con = conn.connect(host='localhost', database='face_data',user='root', password='root', charset='utf8', port=3306)
    cursor = con.cursor()
...
def login():
    ...
    global db
    if(db == []):
        msg = "Guest Mode : You are unknown user first register yourself"
    else:
